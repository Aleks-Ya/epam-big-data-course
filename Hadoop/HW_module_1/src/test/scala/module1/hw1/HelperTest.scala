...
class HelperTest extends FlatSpec with Matchers {
  ...
  it should "parse a data file's line" in {
    ...
    id shouldEqual 20130612000102824L
  }

  it should "write top to file" in {
    val joinedMap = List((1000L, 10), (2000L, 20))
    val file = Files.createTempFile("HelperTest", "tmp")
    Helper.writeToLocalFile(joinedMap, file.toFile)
    val lines = Files.readAllLines(file)
    lines should have size 2
    lines.get(0) shouldEqual "1 1000 10"
    lines.get(1) shouldEqual "2 2000 20"
  }
